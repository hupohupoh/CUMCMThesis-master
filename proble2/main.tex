...
\subsection{物理模型}\label{sec:dose}
...
$m_1$ 与 $m_2$ 本质上都是充电电量，若损伤只取决于电量，二者系数应当相等；而 $m_2$ 的系数更负，说明相同的电量在高 SOC 区间造成的损伤更大，即单位电量的损伤随 SOC 变化。为此引入 SOC 损伤权重函数 $f(\mathrm{SOC})$，将充电电量修正为有效损伤电量：
\begin{equation}\label{eq:fsoc}
m' = C \cdot \int_{s_1}^{s_2} f(s)\,ds,
\end{equation}
则两阶段充电的总有效损伤电量为
\begin{equation}\label{eq:Deff}
D_{\mathrm{eff}} = C_1\cdot F(Q_1) + C_2\cdot\big[F(80)-F(Q_1)\big],
\end{equation}
其中 $F(s)=\int_0^s f(t)\,dt$ 为累积损伤函数，回归方程相应修正为
\begin{equation}\label{eq:reg-fsoc}
\log_{10}(L) = \beta_0 - \beta \cdot D_{\mathrm{eff}}.
\end{equation}

基于 $Q_1$ 分组回归，$f_{\mathrm{high}}>f_{\mathrm{low}}$ 在全部 7 个分组中一致成立，即中高 SOC 区间的损伤权重更大。据此拟合出两种候选函数形式（表 \ref{tab:fsoc}）：线性型形式简洁、便于计算；对数饱和型随 SOC 增长初期上升较快、后期趋于平缓，对应高 SOC 时损伤受传质限制而趋于饱和。

\begin{table}[htbp]
\centering
\caption{两种 $f(\text{SOC})$ 损伤权重函数的拟合结果}
\label{tab:fsoc}
\begin{tabular}{lcc}
\toprule
 & 线性型 & 对数饱和型 \\
\midrule
$f(\text{SOC})$ & $1+0.43\cdot\frac{\mathrm{SOC}}{100}$ & $1+0.21\cdot\ln\!\big(1+\frac{\mathrm{SOC}}{10}\big)$ \\
$F(s)$ & $s+0.21\cdot\frac{s^{2}}{100}$ & $s+0.21\big[(10+s)\ln(1+\frac{s}{10})-s\big]$ \\
$f(0)$ & 1.00 & 1.00 \\
$f(40\%)$ & 1.17 & 1.34 \\
$f(80\%)$ & 1.34 & 1.47 \\
趋势特征 & 线性累积 & 先陡后缓，趋于饱和 \\
...
将第 7.1 节的物理模型展开为策略参数 $C_1$、$Q_1$、$C_2$ 的显式形式。由线性权重函数 $f(\mathrm{SOC})=1+0.43\,\mathrm{SOC}/100$ 得累积损伤函数 $F(s)=s+0.215\,s^2/100$（$s$ 以百分数计），代入式 \eqref{eq:Deff}，总有效损伤电量可写成
\begin{equation}\label{eq:Deff-expand}
D_{\mathrm{eff}} = C_1\Big(Q_1+\frac{0.215\,Q_1^2}{100}\Big) + C_2\Big(80-Q_1+\frac{0.215\,(80^2-Q_1^2)}{100}\Big).
\end{equation}
...
\end{align}
据此逐项分析各参数的影响（数值见表 \ref{tab:sen}）：
\begin{enumerate}
\item \textbf{第一阶段充电倍率 $C_1$。}$\partial D_{\mathrm{eff}}/\partial C_1=F(Q_1)>0$，增大 $C_1$ 始终增加有效损伤、缩短寿命。其边际效应 $F(Q_1)$ 是第一阶段全程 $[0,Q_1]$ 的累积损伤，由于低 SOC 区间权重 $f$ 较小（$f(0)=1$），该值相对有限（样本均值 $F(Q_1)\approx0.50$）；数值上 $C_1$ 每提高 1C，$\log_{10}$ 寿命约下降 0.154（寿命约衰减 30\%）。$C_1$ 的独立效应在统计上常不显著，主要源于低 SOC 区间析锂风险低，以及批次 1 内 $C_1$ 与 $Q_1$ 的强负相关（实验设计混杂）。
\item \textbf{阶段切换 SOC $Q_1$。}$\partial D_{\mathrm{eff}}/\partial Q_1=(C_1-C_2)\,f(Q_1)$，其方向由两阶段倍率的相对大小决定，这是物理模型相对简单回归的关键补充：
\begin{itemize}
...
\end{itemize}
\item \textbf{第二阶段充电倍率 $C_2$。}$\partial D_{\mathrm{eff}}/\partial C_2=F(80)-F(Q_1)>0$，增大 $C_2$ 缩短寿命。其边际为中高 SOC 区间 $[Q_1,80\%]$ 的累积损伤，该区间权重 $f$ 较大（$f(80\%)=1.44$），故 $C_2$ 是倍率类参数中对寿命衰减最直接的负向因素；数值上 $C_2$ 每提高 1C，$\log_{10}$ 寿命约下降 0.136（寿命约衰减 27\%）。
\item \textbf{充电时间 $T_{80}$。}$T_{80}=60\big[Q_1/(100C_1)+(80-Q_1)/(100C_2)\big]$ 是 $C_1$、$C_2$、$Q_1$ 的综合指示：充电越温和、耗时越长，对应的有效损伤剂量越小，寿命越长（偏相关 $+0.363$）。
\end{enumerate}

\begin{table}[htbp]
\centering
\caption{物理模型下各参数的边际影响（standard，$n=89$）}
\label{tab:sen}
\begin{tabular}{lccc}
\toprule
参数 & 边际效应 $\partial\log_{10}L/\partial\text{参数}$ & 观测范围 & 范围内寿命变化 \\
\midrule
$C_1$ & $-0.154$ & 1$\sim$8 C & $\times0.08$ \\
$C_2$ & $-0.136$ & 3$\sim$6 C & $\times0.39$ \\
$Q_1$ & 方向依赖 $C_1-C_2$ & 2$\sim$80\% & 方向依赖 $C_1-C_2$ \\
\bottomrule
\end{tabular}
\end{table}

% 作为补充，经典多变量回归验证了上述结论的方向与显著性。以 $\log_{10}$（循环寿命）为因变量建立含批次效应的回归
% \begin{equation}\label{eq:reg}
% \log_{10}(L) = \beta_0 + \beta_1 C_1 + \beta_2 Q_1 + \beta_3 C_2 + \beta_4 T_{80} + \beta_5 \mathbb{1}_{\mathrm{batch2}} + \varepsilon,
% \end{equation}
% 标准化回归结果见表 \ref{tab:reg}（模型 $R^2=0.674$，不含批次为 0.439）：$C_2$ 为负向效应最显著的倍率参数（标准化系数 $-0.040$，$p=0.025$），$Q_1$（$+0.032$，$p=0.015$）与充电时间 $T_{80}$（$+0.044$，$p=0.001$）显著为正，$C_1$（$-0.021$，$p=0.205$）不显著，与物理模型的边际分析在方向上完全一致。
...
由第 \ref{sec:dose} 节的物理模型可直接回答这一问题：两个阶段的单位电量损伤系数 $|\beta_2|=0.319$ 比 $|\beta_1|=0.266$ 高约 20\%，且 $f(\mathrm{SOC})$ 在全部 7 个 $Q_1$ 分组中一致满足 $f_{\mathrm{high}}>f_{\mathrm{low}}$，即相同电量在中高 SOC 区间造成的寿命损伤显著大于低 SOC 区间。因此，\textbf{不同 SOC 区间内高倍率充电对寿命衰减的影响存在显著差异}，中高 SOC 区间的高倍率充电是寿命衰减的主导因素。

\subsection{各因素影响程度排序}
...
交互项的结果可以统一到物理模型视角理解：$C_1\times Q_1$ 项恰为 $m_1=C_1Q_1/100$ 的常数倍（忽略归一化），其负系数说明"低 SOC 区间高倍率电量的总注入量"越大、寿命越短；$C_2\times Q_1$ 项与 $m_2=C_2(80-Q_1)/100$ 互补——$Q_1$ 增大使 $m_2$ 减小，故 $C_2\times Q_1$ 系数为正。因此，交互项模型与第 7.1 节的物理模型在机理上自洽，从"参数"与"电量"两个视角交叉验证了同一结论。

% ==================== 以下第 7 节后续小节已注释（TODO：后续恢复） ====================
\iffalse
...
校准模型的验证结果见图 \ref{fig:fig13}：预测值与实测值相关系数 $r=0.60$，平均绝对误差 0.49 min（约为均值 11.6 min 的 4.2\%）。$R^2=0.365$ 相对不高，主要原因是\textbf{各策略的充电时间本身集中在 9.6$\sim$13.8 min 的窄区间}（总方差仅 0.66），目标变量变化范围小导致 $R^2$ 天然受限；而模型对"策略 $\rightarrow$ 充电时间"的系统趋势拟合良好（MAE 仅 0.5 min），剩余误差主要来自同策略电池间的个体差异（约 15\% 方差）与高倍率阶段电压限流引起的非线性。对设计阶段的充电时间估计而言，MAE 是更有意义的精度指标，故该模型足以支撑第 9.5 节的优化。同时，这一窄区间也印证了第 6.3 节"充电时间并非寿命差异来源"的结论。
...
Pareto 前沿（图 \ref{fig:fig10}）清晰呈现“充电时间越短、预测寿命越短”的权衡：左端为快而短命的激进策略（$T_{80}\approx9.3$ min、寿命约 280），右端为慢而长寿的温和策略（$T_{80}\approx35$ min、寿命约 2320）。由于寿命在 $\log$ 尺度上随充电时间近似呈“S 形”增长，存在边际收益骤降的“膝点”。采用归一化对数寿命空间中“离端点连线最远的点”作为膝点推荐，得到 3.6C(80\%)-3.6C，恰好落在数据已验证的长寿策略区域，既避免过度追求寿命（充电时间过长）也避免过度压缩时间（寿命过短），是“快充”与“长寿”的平衡选择。
...
\multirow{3}{*}{固定 $T_{80}$} & $\approx 12.0$ min & 4.4C(80\%)-4.4C & 11.9 & 1045 & 1073 \\
 & $\approx 12.5$ min & 4C(80\%)-4C & 12.5 & 1369 & 1570 \\
...
\multirow{4}{*}{固定寿命} & $\approx 500$ & 5.4C(80\%)-5.4C & 10.7 & 531 & 546 \\
 ...
 & $\approx 1400$ & 4C(80\%)-4C & 12.5 & 1369 & 1570 \\
...
\hat{L}=10^{4.312-0.367\,m_1-0.424\,m_2},
\end{equation}
...
\textbf{模型不足与改进。}（1）问题二回归仅解释约 67\%（含批次）的寿命方差，可引入温度、内阻等逐循环协变量提高解释力；（2）问题三长期 SOH 预测精度较低（$t\ge800$ 时 $R^2\approx0.3$），可结合电化学机理或序列模型刻画长期非线性衰减；（3）问题四未考虑电流阶跃、温度约束等工程细节，落地需结合热管理仿真验证。
...
\item \textbf{量化模型}：$C_2$ 是主要的负向倍率策略因素（含批次标准化系数 $-0.040$，偏相关 $-0.250$），充电时间 $T_{80}$ 与寿命正相关（偏相关 $+0.363$）；单位高倍率电量在中高 SOC 区间的寿命损伤比低 SOC 区间高约 16\%（$m_2$ $-0.424$ vs $m_1$ $-0.367$），PyBaMM 析锂仿真从电化学机理层面验证了该 SOC 区间敏感性的物理本质——负极析锂电流密度随 SOC 单调递增。
