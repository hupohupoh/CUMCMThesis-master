...
本数据集为 MIT--Stanford 公开锂离子电池循环老化数据集\upcite{ref1}，含 140 个 A123 18650 磷酸铁锂电池（额定容量 1.1 Ah），分三个批次。充电采用分段快充策略，可表示为：先以 $C_1$ 倍率从 0\% SOC 充电至 $Q_1$，再以 $C_2$ 倍率从 $Q_1$ 充电至 80\% SOC，最后以 1C 恒流--恒压（CC-CV）模式充电至满电。不同电池的差异主要体现于 $C_1$、$Q_1$、$C_2$ 三个参数。
...
全部 138 个有效电池的循环寿命分布见表 \ref{tab:life-dist}，分布直方图见图 \ref{fig:fig1}。寿命跨度达 13 倍（最小值 148、最大值 1935），说明充电策略对寿命具有极显著影响。从分布形态看，寿命整体呈右偏（长尾位于 1000$\sim$1935），中位数 787 显著低于均值水平，反映存在一批寿命特别短（$<300$）的激进策略电池拉低了整体；同时分布并非单峰，而是由三个批次混合而成，各批次寿命重心明显不同。这种"批次混合 + 策略驱动"的分布特征，决定了后续分析必须同时考虑策略参数与批次效应。
...
为刻画策略参数与寿命的整体关联结构，计算各参数间及与寿命的 Pearson 相关矩阵（图 \ref{fig:fig12}）。结果显示：$C_2$ 与寿命呈最强负相关（$r=-0.56$），$Q_1$ 与寿命正相关（$r=0.35$），而 $C_1$（$r=0.21$）与充电时间（$r=0.11$）与寿命的相关较弱。参数之间的相关性总体较弱（$|r|<0.4$），其中 $C_1$ 与 $Q_1$ 的整体相关仅 $-0.06$，说明跨批次合并数据时各参数并非简单线性纠缠，其独立效应需依赖第 7 节的多变量回归解耦。但需注意，这一弱相关是跨批次合并的结果：在批次 1 内，$C_1$ 与 $Q_1$ 呈强负相关（$r=-0.85$），高 $C_1$ 策略普遍设置低 $Q_1$，因而批次 1 内难以独立识别二者的效应；而批次 2 内二者近乎不相关（$r=0.27$）。这一"批次内局部混杂、跨批次整体正交"的结构，正是批次 1 与合并回归中 $C_1$、$Q_1$ 系数符号差异的原因，也进一步说明必须在多变量框架下结合批次效应建模。

图 \ref{fig:fig1} 的箱线图进一步显示：批次 2 的寿命分布整体下移（中位数 483）且跨度大，其中包含大量 $C_2\ge5$C 的激进攻略；批次 1 与批次 3 的寿命中位数接近（858 与 1006），但分布形态不同。综合相关矩阵与分布特征可见，寿命差异主要由 $C_2$ 等策略参数系统驱动，而非随机噪声，这为后续定量建模提供了基础。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.72\textwidth]{figures/fig12.png}
\caption{策略参数与循环寿命的相关性矩阵（standard，n=94）}
\label{fig:fig12}
\end{figure}
...
\caption{充电时间分析：随 $C_2$ 档位分布与理想模型对比（standard，n=94）}
\label{fig:fig17}
\end{figure}

\subsection{批次效应可视化}

批次效应是本数据集的显著特征。图 \ref{fig:fig21} 对比了名义相同策略在不同批次的 SOH 衰减：批次 1 的 4C(80\%)-4C 与批次 2 的 80\%)-4C（解析后均为 $C_1=C_2=4$C、$Q_1=80\%$）策略参数一致，但批次 2 电池在约 200$\sim$500 循环即衰减至 80\% SOH，而批次 1 电池寿命超过 1200 循环。批次 2 的 80\%)-3.6C 亦明显短于批次 1 的同类策略。这表明批次间存在系统性差异（温控、夹具、工艺批次一致性等），因此后文的策略比较一律在同批次内进行，跨批次结论仅作为相对参考。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.72\textwidth]{figures/fig21.png}
\caption{名义相同策略在不同批次的 SOH 衰减对比（批次效应）}
\label{fig:fig21}
\end{figure}
...
从物理机制上解释上述排序：$C_2$ 对应的第二阶段恰好覆盖负极最接近析锂电位的中高 SOC 区间，其偏相关系数绝对值最大，说明在剥离其他因素与批次差异后，$C_2$ 仍是最强的寿命缩短因素；充电时间 $T_{80}$ 的偏相关为正，反映"充电越温和（耗时越长）寿命越长"的残余效应；$C_1$、$Q_1$ 的偏相关较弱，与其覆盖的低 SOC 区间析锂风险较低、且受批次内参数范围限制有关。这一排序回答了问题二要求的"各因素影响程度排序"。
...
从电化学机理看，这一"区间差异"源于负极析锂风险随 SOC 的非单调变化：低 SOC 区间负极电位较高、嵌锂位点充足，析锂过电位裕度大，短暂高倍率冲击造成的锂沉积与电解液副反应有限；而中高 SOC 区间负极表面锂浓度接近饱和，析锂电位窗口收窄，相同的高倍率电量注入会显著加剧析锂与 SEI 膜生长。因此，$m_1$、$m_2$ 系数之比（约 1.27）定量刻画了"同一安时数的高倍率电量，在 SOC 轴上的位置决定了其对寿命的伤害程度"。该剂量模型为后续优化提供了清晰的物理依据：**应尽量将高倍率充电安排在低 SOC 区间，而中高 SOC 区间保持温和倍率**。
...
\caption{不同 $C_2$ 档位下的循环寿命分布（standard，n=94）}
\label{fig:fig16}
\end{figure}
...
校准模型的验证结果见图 \ref{fig:fig13}：预测值与实测值相关系数 $r=0.69$，平均绝对误差 0.50 min。该模型可在设计阶段对不同策略的充电时间进行可靠估计，误差主要来源于高倍率阶段实际电流受电压窗口限制而产生的非线性，属于可接受的工程近似。
...
图 \ref{fig:fig15} 直观对比了推荐策略、备选策略与典型长/短寿命策略的实测寿命与充电时间。推荐策略 3.6C(80\%)-3.6C 在充电时间几乎不增加的前提下，实测寿命比短寿命策略（1C(4\%)-6C 的 300、2C(10\%)-6C 的 148）高出 4$\sim$8 倍，体现了“不牺牲充电时间的前提下显著延长寿命”的优化效果；备选策略 4C(80\%)-4C 进一步将充电时间压缩至 12.6 min，寿命仍高达 1226。
